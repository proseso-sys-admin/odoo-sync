...
export async function loadRoutesFromOdoo(sourceCfg) {
  ...
  for (const t of generalTasks) {
    ...
    const docSync = t.x_studio_odoo_document_sync === true || String(t.x_studio_odoo_document_sync || '').toLowerCase() === 'true';
    if (!docSync) {
      console.log('[routes] project', pid, '→ General task document sync disabled (x_studio_odoo_document_sync)', t.x_studio_odoo_document_sync);
      continue;
    }
