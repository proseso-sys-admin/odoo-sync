...
Routing is read from the source Odoo **General** task (Sync tab): `x_studio_enabled`, `x_studio_odoo_document_sync`, `x_studio_email`, `x_studio_api_key`, `x_studio_multi_company`, `x_studio_company_id_if_multi_company`, `x_studio_accounting_database`. Both `x_studio_enabled` and `x_studio_odoo_document_sync` must be true for a project to get a route.
